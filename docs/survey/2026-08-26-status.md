...
**The companion to [2026-08-26.md](2026-08-26.md), and the only file of the two
that is edited.** That document opens by saying it is the survey as written,
that it is
...
**Both files carry the date the sweep was written rather than the word
`next`.** They were `docs/next.md` and `docs/next-status.md` until a second
sweep became likely, and "next" is a word only one of them can hold — the
citations in `engine/`, `tests/` and `bench/` that point at a section number
have to say *which* survey, or the second one silently reassigns them. So the
folder is `docs/survey/`, a sweep is `<date>.md`, and its state is
`<date>-status.md`.
...
- **A row per item, and the commit that closed it.** Nothing is deleted when it
  ...
  the argument for doing it is still in `2026-08-26.md` and somebody will want
  to know whether it was taken.
- **What an item returned lives here, not in `2026-08-26.md`.** Three of the
  survey's items exist to produce a number or settle a question. Those answers
  are the product, and a commit message is not where anybody looks for them.
...
| **6** The four decisions `2026-08-26.md` does not make | two made — the third, then the first; two still unmade | `f411e24`, `c8176ef` |
...
but the seam does not have to answer it, because `2026-08-26.md` §6 named the
other option in its own words and that option turned out to be the whole of the
work:
...
the seam had to wait for it. `2026-08-26.md` §6 had already written the
alternative — cut the seam above the container and defer the format — and once
that is the
...
1. **The reference machine. Made, and it is half of what the bar asks for** —
   `c8176ef`. `PHILOSOPHY.md`, Performance now defines "the low tier", the
...
Recorded here because `2026-08-26.md` reads as written, the way `docs/review/`
does.
