...
4. **Optional: delete what nothing calls.** The client grep has been run, and it moves two items off this list and one onto firmer ground. `to_radians`, `to_degrees` and `mattmath::clamp` have zero client callers — the two `clamp(` hits in ColourWars are both `std::clamp` at `game/objects/interface_gameplay.cpp:101,137`. `inflate()` has none either, in the client or here. The client names no pairwise free predicate at all, so `intersects.{h,cpp}` and roughly 670 lines of tests are candidates for outright deletion on the strength of both repositories agreeing — subject to the engine's own internal uses, which the step-2 rewrite table enumerates. **`TriangleRightAxisAligned` is off the list:** ColourWars uses it twice, at `game/objects/level_object_builder.cpp:180` and `:202`, the second constructing one into a `unique_ptr` held polymorphically. It is live client API and stays. A deletion is much easier to argue about when the thing to delete is one file rather than a diagonal slice of two, which is a large part of the reason to file them together first.
