...
**The cross-repository caveat was the one thing that could stop the work, and it has been run.** If any ColourWars type derived from `mattmath::Shape` it would implement all seven `intersects` overloads with `override`, and removing the base declarations would be a hard compile error there rather than a silent change. The client is checked out at `C:\VSProjects\ColourWars` (submodule pin `8e43d72`, older than this revision), and grepping `game/` with `external/labrador/` excluded settles it:

- **No client type derives from `Shape`.** Zero hits for `: public mattmath::Shape` and `: public Shape`. Nothing overrides the seven virtuals, so deleting them breaks no client declaration. This repository is likewise clean — the only `: public Shape` sites are `matt_math.h:294, 605, 640, 709, 786`.
- **One `.intersects(` call site in the whole client**, `game/objects/structure_paintable.cpp:116`, `paint_tile.bounds().intersects(splash)`. `splash` is a `RectangleF` (`:72`) and so is `bounds()`, and the comment above it says so — "A tile and a projectile are both axis-aligned rectangles, so it is the exact test." That resolves to `RectangleF::intersects(const RectangleF&)`, which is the first of the three members the surgery explicitly keeps. It compiles unchanged.
- **No `AABB_intersects` call site**, and the client names no pairwise free predicate at all.

The client does hold shapes polymorphically — `std::make_unique<TriangleRightAxisAligned>(triangle)` at `game/objects/level_object_builder.cpp:202`, and `other.shape()->bounding_box()` at `structure_paintable.cpp:72` — but only ever calls `bounding_box()` through the base. Using `Shape` as a base is not the same as deriving from it or dispatching on it, and only the latter two are what the surgery removes.

So the umbrella-for-one-release fallback is not needed and should not be written. Re-run the three greps if the client moves substantially before the surgery lands; as of this revision the answer is that step 2 is source-compatible with ColourWars.
...
2. **The `matt_math` type surgery.** Three files touched — `matt_math.h`, `matt_math.cpp`, and whichever of the nine new test files hold the affected cases — net roughly −300 lines. No file moves. Judgement: the 26-site rewrite table above must be applied exactly, and the `RectangleRotated::contains` ruling honoured. Medium size, high care. **The ColourWars gate is cleared** — see Finding 1; no client type derives from `Shape`, and the client's one `.intersects(` call is the `RectangleF`/`RectangleF` member the surgery keeps. Run `MattMathTests` and diff the output against the pre-surgery run here, so that if `/fp:precise` behaviour differs there is one candidate cause.
...
4. **Delete what nothing calls — done in part, and the rest is a bigger question than this step assumed.** Both repositories were measured, symbol by symbol.

   **Taken:** `to_radians` and `to_degrees`, which had zero references anywhere beyond their own declaration and definition — not a caller, not a test; and `int clamp(int, int, int)`, because every `clamp` in either repository is the float one (`sound_bank.cpp` ×6, `vector2f.cpp:158`, and ColourWars' two hits are `std::clamp`). `float clamp`, `lerp` and `are_equal` stay: `lerp` is in `camera.cpp` and `are_equal` is called by the client at `game/objects/player.cpp`.

   **`TriangleRightAxisAligned` is off the list.** ColourWars uses it at `game/objects/level_object_builder.cpp:180` and `:202`, the second constructing one into a `unique_ptr` held polymorphically. Live client API.

   **Not taken, and this is the finding:** all twenty-five pairwise predicates in `intersects.{h,cpp}` have no caller outside that file and the test suite — no engine, bench, sample or client use whatsoever. The collision module never adopted them: `narrow_phase.cpp` builds its own `Polygon` and runs its own separating-axis test, and `narrow_phase.cpp:175` says in as many words that a circle axis "is not implemented". `broad_phase` and `scene` use `RectangleF::intersects`; `contacts` uses `Shape::AABB_intersects`. So on the evidence the whole table is dead surface and T1 says delete it.

   What stops this being a clean 670-line deletion is what it drags behind it. `Circle` has no consumer either once the predicates go — it is named only by `intersects` and the tests, and the collision system explicitly cannot use it — so `circle.{h,cpp}` follows, and `test_circle_AABB`, `test_circle_circle` and `test_circle_triangle` in `ericson_math` follow that. Deleting only `intersects.{h,cpp}` leaves `Circle` orphaned in the tree, which is a worse state than either end of the argument. Doing it consistently is roughly 1,400 lines of source and tests, including degenerate-case contracts — NaN propagation, zero-length segments, collinear triangles — that both review rounds worked on and that `docs/review/round-2` still cites.

   That is a decision about what the library is for, not a filing question, and it wants an explicit answer rather than an inference from one client's current call sites. Recorded here rather than acted on. If the answer is "delete", the order is `intersects` → `circle` → the three Ericson circle routines, in one commit, because any prefix of that leaves an orphan.

   **Answered 2026-08-15: keep, and the deletion is off the table.** The explicit answer this asked for is that an intersection test a game needs belongs in the engine — a client that has to write one itself is supplying mechanism, and its copy is a second definition of engine arithmetic living outside the tests that hold the degenerate cases. That reason does not move with the call count, which is what makes it an answer rather than another measurement. The reasoning is in `intersects.h` where a reader of the code will meet it; this paragraph is the plan step closing.

   **The measurement above is also now false, and that is the more useful half.** ColourWars calls `rectangle_point_intersect` (`game/objects/level_loaded_info.cpp:74`), reached when `RectangleF::contains(const Point2F&)` was deleted in `42f1f47` and the client migrated onto the surviving free predicate. Zero callers became one with no change to this file. It was taken correctly, in both repositories, and went stale anyway — so a caller count is not a thing a later decision can cite, only a thing a decision can be taken beside. PHILOSOPHY's public-face section carries the general rule.
