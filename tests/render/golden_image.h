...
// them portable and exactly what makes them blind. Every term the two real
// backends hand-copied from each other - the pixels-to-clip constant, the index
// loop, the HLSL and its GLSL transliteration, draw_text's camera prologue -
// is a term both copies can get wrong in the same direction while every
// relationship in that file still holds.
//
...
// two runs, two configurations, one set of images, and a difference is a file
// somebody has to look at rather than an argument somebody has to make.
namespace golden
{
